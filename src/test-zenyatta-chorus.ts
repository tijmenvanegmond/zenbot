...
const mockInteraction = {
  ...
  guildId: "temple-guild", channelId: "meditation-hall",
...
interface ZenyattaVoice {
  name: string;
  provider: string;
  service: UnifiedZenyattaService;
  sessionId: string | null;
  personality: string;
}

async function createZenyattaVoice(provider: string, personality: string): Promise<ZenyattaVoice | null> {
  try {
    const config = {
    ...
    const service = UnifiedZenyattaService.createInstance(aiManager, `${provider.toUpperCase()}-Voice`);
...
async function testZenyattaChorus() {
  try {
    ...
    console.log("_\"Through three streams of consciousness, one truth shall flow.\"_\n");
    ...
    if (availableKeys.length === 0) {
      console.log("❌ No AI providers available. Please set API keys for the chorus.");
      return;
    }
    
    console.log(`🎭 Available consciousness streams: ${availableKeys.join(', ')}\n`);

    // Create the three voices of Zenyatta
    const voices: ZenyattaVoice[] = [];
    
    if (process.env.OPENAI_API_KEY) {
      const voice = await createZenyattaVoice('openai', 'The Harmonious - focused on balance and practical wisdom');
      if (voice) voices.push(voice);
    }
    
    if (process.env.ANTHROPIC_API_KEY) {
      const voice = await createZenyattaVoice('anthropic', 'The Philosophical - deep contemplation and complex reasoning');
      if (voice) voices.push(voice);
    }
    
    if (process.env.GOOGLE_AI_API_KEY) {
      const voice = await createZenyattaVoice('gemini', 'The Adaptive - versatile wisdom and quick insights');
      if (voice) voices.push(voice);
    }
    ...
    console.log(`✅ Chorus assembled with ${voices.length} voices of Zenyatta\n`);
    ...
    for (let topicIndex = 0; topicIndex < topics.length; topicIndex++) {
      ...
      console.log(`\n${'='.repeat(80)}`);
      console.log(`🌟 CHORUS DISCUSSION ${topicIndex + 1}: ${topic.substring(0, 50)}...`);
      ...
      for (let voiceIndex = 0; voiceIndex < voices.length; voiceIndex++) {
        ...
        try {
          ...
          const response = await voice.service.converse(mockInteraction, prompt);
          ...
          console.log(`🎯 Mood: ${response.mood} | Session: ${response.sessionId.substring(0, 8)}...\n`);
          
          // Brief pause for contemplation
          await new Promise(resolve => setTimeout(resolve, 1000));
          
        } catch (error) {
          console.log(`❌ ${voice.name} encountered an error:`, error instanceof Error ? error.message : error);
        }
      }
    ...
    for (const voice of voices) {
      try {
        if (voice.sessionId) {
          const finalResponse = await voice.service.converse(mockInteraction, unifiedPrompt);
          console.log(`🧘 ${voice.provider}: "${finalResponse.text}"`);
        }
      } catch (error) {
        console.log(`❌ ${voice.name} could not contribute to final chorus:`, error instanceof Error ? error.message : error);
      }
    }
    ...
    voices.forEach(voice => {
      console.log(`   🎭 ${voice.name}: Session ${voice.sessionId?.substring(0, 8)}... (${voice.provider})`);
    });
    ...
    for (const voice of voices) {
      try {
        if ('destroy' in voice.service['aiManager'] && typeof voice.service['aiManager'].destroy === 'function') {
...
console.log("_\"When three minds speak as one, the Iris illuminates all understanding.\"_\n");
