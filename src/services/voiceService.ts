...
export class VoiceService {
  // Voice line data
  static readonly VoiceLineDataCollection: VoiceLineData[] =
    adviceList.legacy.data;
  static readonly VoiceLineCategories: VoiceLineCategories =
    adviceList.categories;
   ...
   */
  static getRandomVoiceLineFromCategory(
    category: VoiceLineData[],
  ): VoiceLineData {
   ...
   */
  static getContextualAdvice(context?: AdviceContext): VoiceLineData {
    switch (context) {
      case "greeting":
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.greetings,
        );
      case "philosophical":
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.philosophical,
        );
      case "harmony":
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.orbOfHarmony,
        );
      case "discord":
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.orbOfDiscord,
        );
      case "transcendence":
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.transcendence,
        );
      default:
        return this.getRandomVoiceLineFromCategory(
          this.VoiceLineCategories.philosophical,
   ...
   */
  static async createVoiceLineResource(
    voiceLine: VoiceLineData,
  ): Promise<AudioResource> {
    logger.info(
      `Processing voice line: "${voiceLine.text}", voiceUri: ${voiceLine.voiceUri ? "available" : "null"}`,
    );
    ...
    if (voiceLine.voiceUri) {
      try {
        logger.info(
          `Attempting to play voice line from: ${voiceLine.voiceUri}`,
        );

        // Fetch the audio file from the URL
        const audioBuffer = await new Promise<Buffer>((resolve, reject) => {
          const client = voiceLine.voiceUri!.startsWith("https:")
            ? https
            : http;
        ...
        logger.warn(
          `Error fetching voice line file: ${error}, falling back to TTS`,
        );
      }
    }
  ...
  static async playVoiceLineInChannel(
    ...
    try {
      ...
      await this.playResourceInChannel(channel, resource);
      logger.info(
        `Voice line played in channel ${channel.name}: "${voiceLine.text}"`,
      );
    } catch (error) {
