...
try {
  ...
  console.warn(
    "⚠️ @anthropic-ai/sdk not installed - Anthropic provider disabled",
  );
}
...
export class AnthropicProvider extends BaseAIProvider {
  ...
  constructor(
    ...
    if (!Anthropic) {
      throw new AIServiceError(
        "Anthropic SDK not available. Install with: npm install @anthropic-ai/sdk",
        this.name,
        "SDK_NOT_AVAILABLE",
      );
    }
  ...
  async continueConversation(
    ...
    try {
      ...
      logger.info(
        `💬 Anthropic conversation in session ${sessionId}: ${response.substring(0, 100)}...`,
      );
      return response;
    } catch (error) {
      logger.error(
        `❌ Anthropic conversation failed in session ${sessionId}:`,
        error,
      );
  ...
  async *streamConversation(
    ...
    try {
      ...
      logger.error(
        `❌ Anthropic streaming failed in session ${sessionId}:`,
        error,
      );
      yield {
        type: "error",
