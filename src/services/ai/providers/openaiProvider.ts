...
export class OpenAIProvider extends BaseAIProvider {
  ...
  constructor(sessionStorage: SessionStorage, config: { apiKey: string; baseURL?: string } & Record<string, any>) {
    ...
    if (!config.apiKey) {
      throw new AIServiceError('OpenAI API key is required', this.name, 'MISSING_API_KEY');
    }
  ...
  async continueConversation(
    ...
    try {
      ...
      logger.info(`💬 OpenAI conversation in session ${sessionId}: ${response.substring(0, 100)}...`);
      return response;
      
    } catch (error) {
      logger.error(`❌ OpenAI conversation failed in session ${sessionId}:`, error);
  ...
  async *streamConversation(
    ...
    try {
      // Stream response
      for await (const event of this.generateConversationStream(openaiMessages, {
        ...options,
        model: options.model || session.metadata.model,
        temperature: options.temperature || session.metadata.temperature
      })) {
      ...
      logger.error(`❌ OpenAI streaming failed in session ${sessionId}:`, error);
      yield { 
        type: 'error', 
        metadata: { 
          error: error instanceof Error ? error.message : 'Unknown error' 
  ...
  async callFunctions(
    ...
    try {
      ...
      if (!message) {
        throw new AIServiceError('No response received', this.name, 'EMPTY_RESPONSE');
      }
