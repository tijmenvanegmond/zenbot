...
const mockInteraction = {
  user: { id: "claude-test", username: "claudetester", displayName: "Claude Tester" },
...
async function testClaude() {
  try {
    ...
    if (!process.env.ANTHROPIC_API_KEY) {
      ...
      console.log("To test Claude, set: export ANTHROPIC_API_KEY=your_key_here");
      return;
    }
    ...
    const session = await aiManager.getProvider().getSession(response1.sessionId);
    if (session) {
    ...
    if (error instanceof Error) {
      if (error.message.includes('credit balance')) {
        console.log("💡 Hint: Your Anthropic account needs credits to use Claude");
