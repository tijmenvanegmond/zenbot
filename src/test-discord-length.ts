    ...
    for (const question of testQuestions) {
      ...
      try {
        ...
        if (response.text.length > 500) {
          console.log(`⚠️ Warning: Response is ${response.text.length} chars (might be too long for Discord)`);
        } else if (response.text.length > 200) {
          console.log(`📏 Response is ${response.text.length} chars (acceptable length)`);
        } else {
          console.log(`✅ Response is ${response.text.length} chars (perfect for Discord)`);
        }
