...
async function testAllProviders() {
  try {
    ...
    if (process.env.ANTHROPIC_API_KEY)
      availableProviders.push("Claude/Anthropic");
    ...
    console.log(
      `🧘 Initialized multi-provider system with ${availableProviders.length} providers`,
    );
    ...
    Object.entries(health).forEach(([provider, isHealthy]) => {
      console.log(
        `${isHealthy ? "✅" : "❌"} ${provider}: ${isHealthy ? "Healthy" : "Unavailable"}`,
      );
    });
    ...
    const healthyProviders = Object.entries(health)
      ...
      .map(([name]) => name);
    console.log(
      `🎯 ${healthyProviders.length} healthy providers: ${healthyProviders.join(", ")}`,
    );
    ...
    for (const provider of healthyProviders) {
      ...
      try {
              ...
              metadata: {
                ...
                model:
                  multiConfig.providers[
                    provider as keyof typeof multiConfig.providers
        ...
        if (response) {
          ...
          console.log(
            `🧘 Mood:`,
            typeof response === "string" ? "N/A" : response.mood,
          );
        }
      } catch (error) {
        console.log(
    ...
    for (const provider of healthyProviders) {
      ...
      try {
        ...
        if (aiManager.getProvider(provider).supportsStreaming) {
          ...
          const testSession = await aiManager.createSession("Zenyatta", {
            userId: `stream-test-${provider}`,
          });

          for await (const event of aiManager.chatStream(
            testSession.id,
            "Give me a very short Zenyatta quote about peace.",
          )) {
            eventCount++;
          ...
          console.log(
            `  📊 Stream events: ${eventCount}, Content length: ${streamContent.length}`,
          );
        } else {
        ...
        console.log(
          `  ${aiManager.getProvider(provider).supportsFunctions ? "✅" : "❌"} Function calling: ${aiManager.getProvider(provider).supportsFunctions ? "Supported" : "Not supported"}`,
        );

        // Test vision
        console.log(
          `  ${aiManager.getProvider(provider).supportsVision ? "✅" : "❌"} Vision: ${aiManager.getProvider(provider).supportsVision ? "Supported" : "Not supported"}`,
        );

        // Test supported models
        const models = aiManager.getProvider(provider).supportedModels;
        console.log(
          `  🎯 Models: ${models.slice(0, 3).join(", ")}${models.length > 3 ? ` (+${models.length - 3} more)` : ""}`,
        );
      } catch (error) {
        console.log(
          `  ❌ Capability test failed: ${error instanceof Error ? error.message : error}`,
        );
      }
    }
    ...
    Object.entries(multiConfig.fallbacks.providers).forEach(
      ([primary, fallbacks]) => {
        if (health[primary]) {
          console.log(
            `🔄 ${primary} → ${fallbacks.length > 0 ? fallbacks.join(" → ") : "STOP"}`,
          );
        }
      },
    );
