...
export default async function voiceRoutes(
  ...
  fastify.get(
    "/guilds/:guildId/voice-channels/active",
    async function handler(request: any, reply) {
      const { guildId } = request.params;
  ...
  fastify.get(
    "/guilds/:guildId/voice-channels/:channelId/members",
    async function handler(request: any, reply) {
  ...
  fastify.post(
    "/guilds/:guildId/voice-channels/:channelId/join",
    async function handler(request: any, reply) {
      const { guildId, channelId } = request.params;
  ...
  fastify.post(
    "/guilds/:guildId/voice-channels/:channelId/leave",
    async function handler(request: any, reply) {
      const { guildId, channelId } = request.params;
      ...
      if (!connection) {
        reply.code(400).send({
          error: "Bot is not connected to any voice channel in this guild",
        });
        return;
      }
  ...
  fastify.get(
    "/guilds/:guildId/voice-channels/current",
    async function handler(request: any, reply) {
      const { guildId } = request.params;
      ...
      const currentChannel = guild.channels.cache.get(
  ...
  fastify.get("/tts/voices", async function handler(request: any, reply) {
    const voices = [
      ...
      { id: "onyx", name: "Onyx", description: "Deep and resonant" },
      {
        id: "nova",
        name: "Nova",
        description: "Bright and energetic (default)",
      },
