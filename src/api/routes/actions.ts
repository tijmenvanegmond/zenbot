...
export default async function actionsRoutes(
  ...
  fastify.get("/actions", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const actions = allActions.map(action => ({
        ...
        allowedSources: action.permissions?.allowedSources || ["command", "voice", "api", "ai"],
        requiresVoiceChannel: action.permissions?.requiresVoiceChannel || false,
        schema: action.schema,
        aiEnabled: !action.permissions?.allowedSources || 
   ...
   */
  fastify.get("/actions/categories", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const allActions = actionRegistry.getAll();
   ...
   */
  fastify.get<{ Params: { actionName: string } }>("/actions/:actionName", 
    async (request, reply) => {
      try {
        ...
        logger.error(`Failed to fetch action details for ${request.params.actionName}:`, error);
        return reply.code(500).send({
          success: false,
   ...
   */
  fastify.get("/actions/ai", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post<{ 
    ...
    try {
      ...
      if (action.permissions?.allowedSources && 
      ...
      logger.error(`🎭 API action execution failed for ${request.params.actionName}:`, error);
  ...
  fastify.post<{ 
    ...
    try {
      ...
      return {
        ...
        wisdom: isValid ? "Parameters align with expectations" : "Refinement brings clarity"
      };

    } catch (error) {
      logger.error(`Failed to validate parameters for ${request.params.actionName}:`, error);
      return reply.code(500).send({
        success: false,
